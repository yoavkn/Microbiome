...
def combine_human_and_bacteria(directory):
    human_fasta = f"{directory}\\Human_Top_100.fasta"
    combined_bacteria = f"{directory}\\combined_proteome.fasta"
...
def top20_main_function(raw_list, organisms, directory, chosen_fastas_folder):
    ...
    with open(rf"{chosen_fastas_folder}\combined_proteome.fasta", 'a') as combined_proteome_file, open(
        ...
        for organism_name in organisms:
            ...
            all_raws_non_human_peptides_dict = {}
            ...
            for raw_name in raw_list:
                ...
                non_human_peptides_dict = fasta_non_human_protein_dict(peptides_dictionary, df_dictionary,
                                                                       CONTAM_NOTATION)
                all_raws_non_human_peptides_dict[raw_name] = non_human_peptides_dict

                for fasta in non_human_peptides_dict.keys():
                    if fasta in fasta_peptides_count:
                        fasta_peptides_count[fasta] += len(non_human_peptides_dict[fasta])
                    else:
                        fasta_peptides_count[fasta] = len(non_human_peptides_dict[fasta])
            ...
            for raw_name in raw_list:
                if chosen_proteome in all_raws_non_human_peptides_dict[raw_name]:
                    found_organism_proteins_names.update(all_raws_non_human_peptides_dict[raw_name][chosen_proteome])
            ...
            organism_fasta_folder = rf"{chosen_fastas_folder}\{organism_name.replace('+', '_')}_{chosen_proteome}"
        ...
        # combine_human_and_bacteria(chosen_fastas_folder) todo: check
