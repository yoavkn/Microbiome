...
def combine_human_and_bacteria(new_proteomes_folder, proteomes_fasta_list, organism_name):
    for prot in proteomes_fasta_list:
        new_folder_name = rf"{new_proteomes_folder}\{organism_name.replace('+','_')}\{prot}"
...
def download_organisms_main(fragpipe_directory, directory, raw_list, organisms, res_folder, number_of_fastas_download):
    """
    ...
    for organism_name in organisms:
        cur_organism_folder = rf'{res_folder}\{organism_name.replace("+", "_")}'
        ...
        if os.path.exists(cur_organism_folder):
            ...
            combine_human_and_bacteria(res_folder, proteomes_fasta_list, organism_name)
        ...
        for raw_name in raw_list:
            completed_analysis = multiple_run_msfragger_runner(fragpipe_directory, directory, res_folder, proteomes_fasta_list,
